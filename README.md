...
prototype for a VW/Audi specialist DSG and S tronic calibration workstation. The current app is a single-page HTML control view for modeling drivability changes before any real bench or vehicle process.
...
When the simulated oil temperature reaches 130°C, the Abuse index card will glow red.
Contributing & Driver Development
To add a new hardware driver:

Inherit from TcuDriver in src/vw_tcu_calibrator/drivers/base.py.
Implement the read_did method using your hardware's SDK (e.g., J2534, SocketCAN).
Register the driver in the toolkit CLI.
...
Connect approved hardware to the car or bench harness.
Identify vehicle, TCU family, voltage state, and communication health.
Import baseline logs and calibration project files.
Tune high-level map targets for drivability, protection, and shift quality.
Run safety checks before export.
Generate a review pack for the tuner, customer, and bench technician.
Only perform programming through lawful, authorized tooling and validated procedures.
...
Add A Driver
Add a driver file under src/vw_tcu_calibrator/drivers/.
Subclass BaseReadOnlyDriver.
Implement connect(), close(), and send(arbitration_id, payload).
Register the driver name in drivers/registry.py.
Add a sample command to README_TOOLKIT.md.
Add or update tests proving write/protected services are blocked in read_only mode.
Add any new public DID scale/offset data to data/dq500_map.json with a validation status.
Safety Rule
